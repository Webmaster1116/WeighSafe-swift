...
enum ValidatorType {
    ...
    case rangeField(min: Double, max: Double, field: String)
    case age
}
...
enum VaildatorFactory {
    static func validatorFor(type: ValidatorType) -> ValidatorConvertible {
        ...
        case .rangeField(let minValue, let maxValue, let fieldName): return RangeValidator(minValue, maxValue, fieldName)
...
struct RangeValidator: ValidatorConvertible {
    private let minValue: Double
    private let maxValue: Double
    private let fieldName: String
    
    init(_ min: Double, _ max: Double, _ field: String) {
        minValue = min
        maxValue = max
        fieldName = field
    }
    func validated(_ value: String) throws -> String {
        guard !value.isEmpty else {throw ValidationError("\(fieldName)")}
        guard let convertedValue = Double(value) else {throw ValidationError("\(fieldName) must be a number.")}
        guard convertedValue >= minValue && convertedValue <= maxValue else {throw ValidationError("\(fieldName) must be between \(minValue) and \(maxValue)")}
        return value
    }
}
